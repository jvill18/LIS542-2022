...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                    ...
                    <p>
                        <ol>
                            <xsl:for-each select="//d:entries//d:place">
                                <li>
                                    <xsl:value-of select="."/>  <!-- the . here means to use the content of the selected element; in this case, <place> -->
                                </li>
                            </xsl:for-each>
                        </ol>
                    </p>
    ...
    <xsl:template match="d:p"> <!-- Match <entry>, surround each one with an HTML <p> element and keep going  -->
        [<xsl:value-of select="concat('Paragraph ',translate(@n, 'p0', ''))"></xsl:value-of>] 
        <p>
            <xsl:apply-templates/>
        </p>
    </xsl:template>
    ...
    <xsl:template match="d:cite"> 
        <a>
            ...
            <xsl:apply-templates/>
            <!--Referenced following for additional guidance: https://stackoverflow.com/questions/1454337/construct-a-href-using-xslt-->
        </a>
    </xsl:template>
